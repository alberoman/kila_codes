...
date_initial_str = '2018-06-20'
date_final_str = '2018-08-05'
months = mdates.MonthLocator()  # every month
days = mdates.DayLocator()  # every day
date_fmt = mdates.DateFormatter('%m/%d')
date_initial_plot = mdates.date2num(datetime.strptime('2018-06-20','%Y-%m-%d'))
...
for name in list_station:
    ...
    north = signal.filtfilt(bhigh, ahigh,north)
    east = signal.filtfilt(bhigh, ahigh,east)
...
ax_pl = fig.add_subplot(gs[5:,2:-2])
...
magnitude = (stations['UWD']['east']**2 + stations['UWD']['north']**2)**0.5
filename_counter = 0
for counter in np.arange(0,len(time_tilt),30):
    h1 = []
    h2 = []
    for name in list_station:
        ...
        h2.append(handle2)
    print(counter)
    timeseries1 = ax_pl.plot(time_tilt,magnitude,'blue')
    time_segment = time_tilt[:counter]
    magnitude_segment = magnitude[:counter]
    timeseries2 = ax_pl.plot(time_segment,magnitude_segment,'r')
    ax_pl.set_xlim(date_initial_plot,date_final_plot)
    ax_pl.xaxis.set_major_formatter(date_fmt)
    ax_pl.xaxis.set_major_locator(months)
    ax_pl.xaxis.set_minor_locator(days)
    ax_pl.set_ylabel('UWD [microrad]')
