...
for i in station_indexes.keys():
    ...
    for j in range(len(station_indexes[i])):
        ...
        east_full.append(east_tilt[::10])
        north_full.append(north_tilt[::10])
        t_full.append(time[::10])
        flag_full = np.concatenate((flag_full,flag))
    flag_full[0] = 0       #the last element of the flag is not a concatenation!
    station[i]['time'] = t_full
    station[i]['north'] = north_full
    station[i]['east'] = east_full
pickle.dump(station, open( "tilt_dictionary_01may_discontinuous.pickle", "wb" ))
