...
TSLIP_seed = 1 / timescale
N_cycles = int(np.ceil((R3 - 1) * (R1 + 1) / (2 * (1 - R5) * R1))) - 1  #Il manque juste le dernier collapse
...
for i in np.arange(len(magnitude1)):
    ...
    ax_UWD.set_xlim([np.min(time_tilt),timeMax_axis])
    ...
    ax_SDH.set_xlim([np.min(time_tilt),timeMax_axis])
