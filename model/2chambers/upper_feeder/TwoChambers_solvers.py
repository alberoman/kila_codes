...
def ps_analytic_smallVar(t,R3,T1,phi,pd0,ps0):
    ps = (-R3*T1 + pd0 + np.exp((T1 + 1)*(-t + np.log(R3*T1 - pd0 + ps0*(T1 + 1))/(T1 + 1))))/(T1 + 1)
    return ps
...
def pd_analytic_smallVar(t,R3,T1,phi,pd0,ps0):
    pd = -phi*(T1*t*(R3 + pd0) + (R3*T1 + T1*ps0 - pd0 + ps0)*np.exp(-t*(T1 + 1))/(T1 + 1))/(T1 + 1) + (R3*T1*phi + T1*phi*ps0 - pd0*phi + pd0*(T1 + 1)**2 + phi*ps0)/(T1 + 1)**2
    return pd

def system_2chambers_timein(w0,par,pslip,tslip,time,ps,pd,tsl_seed):
    ps0,pd0 = w0
...
def system_2chambers_smallVar(w0,par,nn):
    ps0,pd0 = w0
    r1,r3,r5,t1,phi = par
    tslip = (np.log((r1 + 1)*(r3*t1 - pd0) - (t1 + 1)*(-2*r1*nn*(r5 - 1) + r1 + 2*r5 - 1)) - np.log(2*r1*nn*(r5 - 1)*(t1 + 1) - (r1 + 1)*(t1 + 1) + (r1 + 1)*(r3*t1 - pd0)))/(t1 + 1)
    tsegment = np.linspace(0,tslip,30)
