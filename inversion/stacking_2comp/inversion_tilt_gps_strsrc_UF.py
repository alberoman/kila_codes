...
with pm.Model() as model:
    ...
    A_mod = pm.Uniform('A_mod',lower = 0, upper = 1e+4)
    B_mod = pm.Uniform('B_mod',lower = 0, upper = 1e+4)
    C_mod = pm.Uniform('C_mod',lower = 0, upper = 1e+4)
    D_mod = pm.Uniform('D_mod',lower = 0, upper = 1e+4)
    E_mod = pm.Uniform('E_mod',lower = 0, upper = 1e+4)
    F_mod = pm.Uniform('F_mod',lower = 0, upper = 1e+4)
    ...
    phi = 1 * Vs_mod / Vd_mod
    #stacky_mod  =  -(A_mod * tt.exp(tstack/tau2*(-T1/2 - phi/2 + tt.sqrt(4*phi + (-T1 + phi - 1)**2)/2 - 1/2)) + B_mod * tt.exp(tstack/tau2*(-T1/2 - phi/2 - tt.sqrt(4*phi + (-T1 + phi - 1)**2)/2 - 1/2))  - E_mod)
    stackx_mod  =   coeffx *1e+6 * pspd_mod *(C_mod * tt.exp(tstack/tau2*(-T1/2 - phi/2 + tt.sqrt(4*phi + (-T1 + phi - 1)**2)/2 - 1/2)) + D_mod * tt.exp(tstack/tau2*(-T1/2 - phi/2 - tt.sqrt(4*phi + (-T1 + phi - 1)**2)/2 - 1/2))  - F_mod)
...
pickle.dump(results,open(path_results + 'res' + str(Niter) + '_strsrc_UF.pickle','wb'))
#pm.save_trace(trace, path_results +'trace'+str(Niter) + '_strsrc_UF',overwrite =  True)
#pm.traceplot(trace)
