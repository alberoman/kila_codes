...
pathMAP = '/Users/aroman/work/kilauea_2018/results/onech/priorsN/UF/UWD/07-03-2018/locErrx10/'
pathrun = 'fromMAP'
...
def parameters_init(mt):
    ...
    if mt == 'UF':
        bounds = np.array([[+8,10],[+9,+12],[+7,+11],[8,11],[dxmin,dxmax],[Ncycmin,Ncycmax],[1,10],[1,20],[0.5,2.0]])
        boundsLoc = np.array([[-1000,1000],[1200,2500],[500,1500],[-1500,+1500],[-1500,2500],[2000,4000]])
    elif mt == 'LF':                                                 
        bounds = np.array([[+8,+12],[+8,+10],[+8,+11],[7,11],[dxmin,dxmax],[Ncycmin,Ncycmax],[1,10],[1,20],[0.5,2.0]])
...
if  os.path.isfile(pathgg + 'progress.h5'):
    ...
    pos,nwalkers,ndim = walkers_init_fromMAP(pathMAP,nwalkers,ndim,bounds,boundsLoc,rhog,S,locTruth,locErr,bndtiltconst,bndGPSconst,bndp0,Nst,model_type,flaglocation)
