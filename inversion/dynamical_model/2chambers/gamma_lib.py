...
def DirectModelEmcee_inv_LF(tOrigTilt,tOrigGPS,
    ...
    R1Samp = rhog * (ksSamp * VdSamp + gammaSamp * kdSamp * VsSamp)/(ksSamp * kdSamp) * 1. / S
    T1 = (condsSamp / conddSamp )**4 * ld /ls
    PHI = kdSamp /ksSamp * VsSamp / VdSamp
    params = [T1,PHI,R3Samp] #R1 is the ratio of the hydraulic parameters (for high values the top conduit is more efficient)
    tstar = VsSamp * 8 * mu * ld / (ksSamp * 3.14 * conddSamp**4)
    xstar = pspdSamp *(ksSamp * VdSamp + gammaSamp * kdSamp * VsSamp)/(ksSamp * kdSamp) * 1. / S
    deltap0adim = deltap0Samp / pspdSamp
    ...
    while i < N_cycles + 1 and i < thresh:
        ...
        PS0 = ps0 + + 4 * alphaSamp / (1 + R1Samp) * gammaSamp
...
def log_likelihood_LF(param,
                   ...
                   tiltErr,GPSErr,nstation):
    if np.max(nstation) ==  0:
        deltap0Samp,offGPSSamp,offx1,offy1,xsSamp,ysSamp,dsSamp,xdSamp,ydSamp,ddSamp,VsExpSamp,VdExpSamp,ksExpSamp,kdExpSamp,pspdSamp,R3Samp,condsSamp, conddSamp,alphaSamp,gammaSamp = param
        offxSamp = np.array([offx1])
        offySamp = np.array([offy1])
    if np.max(nstation) ==  1:
        deltap0Samp,offGPSSamp,offx1,offy1,offx2,offy2,xsSamp,ysSamp,dsSamp,xdSamp,ydSamp,ddSamp,VsExpSamp,VdExpSamp,ksExpSamp,kdExpSamp,pspdSamp,R3Samp,condsSamp, conddSamp,alphaSamp,gammaSamp = param
        offxSamp = np.array([offx1,offx2])
        offySamp = np.array([offy1,offy2])
    if np.max(nstation) == 2:
        deltap0Samp,offGPSSamp,offx1,offy1,offx2,offy2,offx3,offy3,xsSamp,ysSamp,dsSamp,xdSamp,ydSamp,ddSamp,VsExpSamp,VdExpSamp,ksExpSamp,kdExpSamp,pspdSamp,R3Samp,condsSamp, conddSamp,alphaSamp,gammaSamp = param
        offxSamp = np.array([offx1,offx2,offx3])
        offySamp = np.array([offy1,offy2,offy3])
...
def log_prior_LF(param,S,rhog,bounds,boundsLoc,bndGPSconst,bndtiltconst,bndp0,locTr,locEr,nstation,flaglocation):
    ...
    if flaglocation =='N': # Normal priots
        ...
        conditions.append(bounds[9,0] < alphaSamp < bounds[9,1])
        conditions.append(all(np.abs(offs)<bndtiltconst))
        conditions.append(-bndGPSconst < offGPSSamp < 0)
        conditions.append(-bndp0 < deltap0Samp < bndp0)
        if all(conditions):
...
def walkers_init(nwalkers,ndim,bounds,boundsLoc,rhog,S,locTruth,locErr,bndtiltconst,bndGPSconst,bndp0,Nst,mt,flaglocation):
    ...
    if mt == 'UF':
        R1Initial = rhog * Vs /(ks * S)
    elif mt == 'LF':
        R1Initial = rhog * (ks * Vd + gamma * kd * Vs)/(ks * kd) * 1. / S
