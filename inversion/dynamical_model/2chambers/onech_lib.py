...
def log_prior(param,S,rhog,bounds,boundsLoc,bndGPSconst,bndtiltconst,locTr,locEr,nstation,flaglocation):
    if np.max(nstation) ==  0:
        offGPSSamp,offx1,offy1,xsSamp,ysSamp,dsSamp,VsExpSamp,ksExpSamp,pspdSamp,R3Samp,condsSamp,alphaSamp = param
    ...
    if np.max(nstation) == 2:
        offGPSSamp,offx1,offy1,offx2,offy2,offx3,offy3,xsSamp,ysSamp,ddSamp,VsExpSamp,ksExpSamp,pspdSamp,R3Samp,condsSamp,alphaSamp = param
    ...
    if flaglocation =='N': # Normal priots
        ...
        conditions.append(bounds[5,0] < alphaSamp < bounds[5,1])
...
def log_probability(param,
    ...
    lp = log_prior(param,S,rhog,bounds,boundsLoc,bndGPSconst,bndtiltconst,locTruth,locErr,nstation,flaglocation)
