...
def parameters_init():
    ...
    ndim = len(bounds)
    return ls,ld,pt,mu,rhog,const,S,bndtiltconst,bndGPSconst,bndtimeconst,bounds,ndim
...
if  os.path.isfile(pathgg + 'progress.h5'):
    ...
    totaliter = input('How many iteration you want to run? ')
    Nst,nstation,x,y,tTilt,tx,ty,tGPS,GPS,locTruth,locErr,t0 = preparation(stations,date)
    ls,ld,pt,mu,rhog,const,S,bndtiltconst,bndGPSconst,bndtimeconst,bounds,ndim = parameters_init()
    ...
    with Pool() as pool:
        ...
        sampler.run_mcmc(pos,totaliter, progress = True,thin = thin)
